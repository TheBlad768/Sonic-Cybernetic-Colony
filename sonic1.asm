...
Vectors:
		...
		dc.l ErrorTrap,	ErrorTrap, ErrorTrap, ErrorTrap
	if * <> $100
		fatal "StartOfROM was $\{*} but it should be $100"
	endif
Header:			dc.b "SEGA GENESIS    "
Copyright:		dc.b "(C)SEGA 2016.DEC"
Domestic_Name:	dc.b "SONIC CYBERNETIC COLONY                         "
Overseas_Name:	dc.b "SONIC CYBERNETIC COLONY                         "
Serial_Number:	dc.b "GM MK-1542 -03"
Checksum:		dc.w 0
Input:			dc.b "J               "
...
GameModeArray:
		...
		dc.l Splash_Screen		; Splash Screen ($04)
		dc.l Level					; Demo Mode ($08)
		dc.l Level					; Normal Level ($0C)
		dc.l Title_Screen			; Title Screen ($10)
		dc.l Title_Screen			; Title Screen ($14)
		dc.l Title_Screen			; Title Screen ($18)
		dc.l Title_Screen			; Title Screen ($1C)
		dc.l Options_Screen		; Options Screen ($20)
		dc.l Title_Screen			; Title Screen ($24)
		dc.l Credits_Screen		; Credits Screen ($28)
		dc.l Advanced_Screen		; Title Screen ($2C)
		dc.l Credits_Screen		; Credits Screen ($30)
; ===========================================================================
...
loc_B9A:
		...
		move.w	(VDP_control_port).l,d0
		btst	#6,(Graphics_flags).w	; is Sega PAL (European)?
		beq.s	loc_BBA			; if not, branch
		move.w	#$700,d0
		dbf	d0,*			; delay processor

loc_BBA:
		move.w	#1,(H_int_flag).w	; enable HBlank
...
VBlank_Sub08:				; XREF: VBlank_Routines
		...
		beq.s	VBla_08_NoFlash
		subq.b	#1,(Hyper_Sonic_flash_timer).w
		lea	(VDP_data_port).l,a6
		move.l	#$C0000000,VDP_control_port-VDP_data_port(a6)
		move.w	#$EEE,d0
		move.w	#$1F,d1
@1		move.w	d0,(a6)
		dbf d1,@1
...
HBlank:
		...
		tst.w	(H_int_flag).w	; was the pallete set to change?
		beq.s	HBlank_Rts	; if not, branch
...
; ---------------------------------------------------------------------------
; Subroutine to snd driver
; ---------------------------------------------------------------------------

		include "z80 driver/Init.asm"
...
loc_13CA:
		...
		move.b	#$24,($FFFFF600).w ; set game mode to 4 (title screen)
		nop
		bra.s	loc_1404
; ===========================================================================
...
NemDec:
        movem.l d0-a1/a3-a6,-(sp)
        lea     VDP_data_port,a4              ; load VDP Data Port
...
EniDec:
		movem.l	d0-d7/a1-a5,-(sp)	; save registers to the stack
		movea.w	d0,a3
		move.b	(a0)+,d0
		ext.w	d0
		movea.w	d0,a5
...
PalCycle_GHZ:
		...
		bpl.s	@2
		move.w	#6,($FFFFF634).w
		lea	AnPal_GHZ1(pc),a0
		move.w	($FFFFF632).w,d0
		addq.w	#2,($FFFFF632).w
		cmpi.w	#4,($FFFFF632).w
		bcs.s	@1
		move.w	#0,($FFFFF632).w
@1		move.w	(a0,d0.w),($FFFFFB5C).w
@2		rts
...
MusicList:	incbin	misc\muslist1(Z80).bin
	endif
		even
; ===========================================================================
...
loc_37FC:
		...
		moveq	#2,d0
		bsr.w	LoadPLC		; load explosion patterns
...
Level_LoadObj:
		...
		tst.b (Rings_mode_count).w
		beq.s	loc_39E8
		move.w	#10,($FFFFFE20).w ; clear rings
...
loc_39E8:
		...
		move.b	#0,($FFFFFE1E).w ; update time counter
		move.w	#0,($FFFFF790).w
...
DynWater_LZ1:				; XREF: DynWater_Index
		...
		bne.s	loc_3CD0		; if routine > $00, branch
		move.w	#$B8,d1
		cmpi.w	#$600,d0
		bcs.s	loc_3CB4
		move.w	#$108,d1
		...
		cmp.w	($FFFFF648).w,d1
		bne.s	loc_3CB4
		move.b	#1,($FFFFF64D).w	; set routine to $01
...
DynWater_LZ3:				; XREF: DynWater_Index
	...
	if Snd_Driver_Version=0
		move.w	#$B7,d0
		jsr	(PlaySound_Special).l ; play sound $B7 (rumbling)
	endif
...
SpecialStage:				; XREF: GameModeArray
	if Snd_Driver_Version=0
		move.w	#$CA,d0
		jsr	(PlaySound_Special).l ; play special stage entry sound
	endif
		bsr.w	Pal_MakeFlash
		move	#$2700,sr
		lea	(VDP_control_port).l,a6
...
SS_ClrNemRam:
	...
	if Snd_Driver_Version=0
		move.w	#$89,d0
		jsr	(PlaySound).l	; play special stage BG	music
	endif
		move.w	#0,($FFFFF790).w
		lea	(Demo_Index).l,a1
		moveq	#6,d0
...
SS_NormalExit:
	...
	if Snd_Driver_Version=0
		move.w	#$CA,d0
		jsr	(PlaySound_Special).l ; play special stage exit sound
	endif
		bsr.w	Pal_MakeFlash
		rts
; ===========================================================================
...
Cont_ClrObjRam:
	...
	if Snd_Driver_Version=0
		move.b	#$90,d0
		jsr	(PlaySound).l	; play continue	music
	endif
...
End_LoadData:
	...
	if Snd_Driver_Version=0
		move.w	#$8B,d0
		jsr	(PlaySound).l	; play ending sequence music
	endif
	if Debug_Mode=1
...
End_MainLoop:
		...
		cmpi.b	#6,($FFFFFE57).w ; do you have all 6 emeralds?
		beq.s	loc_52D0	; if yes, branch
	if Snd_Driver_Version=0
		move.b	#$95,d0
		jsr	(PlaySound_Special).l ; play BAD Ending credits music
	endif
...
loc_52D0:
	if Snd_Driver_Version=0
		move.b	#$91,d0
		jsr	(PlaySound_Special).l ; play TRUE Ending credits music
	endif
		move.w	#0,($FFFFFFF4).w ; set credits index number to 0
		rts
; ===========================================================================
...
Ani_obj8D:
	include "_anim\obj8D.asm"
Ani_RobotnikRun:
	include "_anim\Anim - Robotnik Run.asm"
...
Map_obj8B:				include "_maps/obj8B.asm"
Map_obj8D:				include "_maps/obj8D.asm"
Map_RobotnikShipCrane:	include "_maps/Map - Robotnik Ship Crane.asm"
Map_RobotnikRun:		include "_maps/Map - Robotnik Run.asm"
...
Deform_GHZ:
		lea	@ParallaxScript(pc),a1
		jmp	(ExecuteParallaxScript).l
; ---------------------------------------------------------------

@ParallaxScript:
		dc.w	_moving|$01, $10, 15
		dc.w	_moving|$00, $10, 15
		dc.w	_moving|$01, $10, 16
		dc.w	_moving|$00, $10, 15
		dc.w	_moving|$01, $10, 15
		dc.w	_moving|$00, $10, 15
		dc.w	_moving|$01, $10, 15
...
MainLoadBlockLoad:
		...
		lea	(a2,d0.w),a2
		move.l	a2,-(sp)
		; Vladikcomper: Load Kosinski-compressed level-art
		lea	(VDP_control_port).l,a6
		move.l	(a2)+,a0		; a0 = Source address
		lea	($FF0000).l,a1	; a1 = Dest address
		bsr.w	KosDec
		move.w	a1,d0
		lsr.w	d0			; d0 = DMA length
		move.l	#$94009300,-(sp)
		movep.w	d0,1(sp)
		move.l	(sp)+,(a6)		; VDP -> set DMA length
		move.w	#$9500,(a6)		; VDP -> set DMA source address LL
		move.l	#$977F9680,(a6)		; VDP -> set DMA source address HHMM
		move.l	#$40000080,(a6)		; VDP -> start DMA at VRAM address $0000
		movea.l	(a2)+,a0
...
Resize_Index:
		dc.w Resize_GHZ-Resize_Index, Resize_LZ-Resize_Index
...
Resize_GHZ2:
		move.w	(Screen_Shaking_Screen).w,d0
		add.w	d0,($FFFFF704).w
		moveq	#0,d0
		move.b	($FFFFF742).w,d0
		move.w	off_6E1A(pc,d0.w),d0
		jmp	off_6E1A(pc,d0.w)
; ===========================================================================

off_6E1A:
		dc.w Resize_GHZ2main-off_6E1A
		dc.w Resize_GHZ2boss-off_6E1A
		dc.w Resize_GHZ2end-off_6E1A
		dc.w Resize_GHZ2boss2-off_6E1A
		dc.w Resize_GHZ2end-off_6E1A
		dc.w Resize_GHZ2_StartMove-off_6E1A
		dc.w Resize_GHZ2_CameraMove-off_6E1A
		dc.w Resize_GHZ2_CameraStop-off_6E1A
; ===========================================================================

Resize_GHZ2main:
		move.w	#$200,($FFFFF726).w
		move.w	#$200,($FFFFF72C).w
		cmpi.w	#$550,($FFFFF700).w
		ble.s		locret_6EDC
		addq.b	#2,($FFFFF742).w
...
Resize_GHZ2_StartMove:
		addq.b	#2,($FFFFF742).w
		move.b	#0,($FFFFF7AA).w
		move.b	#1,($FFFFF7CC).w
		move.w	#$CC0,($FFFFF72A).w

Resize_GHZ2_CameraMove:
		move.w	#$800,($FFFFF602).w
		cmpi.w	#$E30,($FFFFD008).w
		blo.s		Resize_GHZ2_Locret
		addq.b	#2,($FFFFF742).w

Resize_GHZ2_CameraStop:
...
Map_obj3F:	include "_maps/obj3F.asm"
	even
; ===========================================================================
; ---------------------------------------------------------------------------
; Object 28 - animals
...
Obj37_MakeRings:			; XREF: Obj37_CountRings
		move.b	#$37,(a1)	; load bouncing	ring object
		addq.b	#2,$24(a1)
		move.b	#8,$16(a1)
		move.b	#8,$17(a1)
		move.w	8(a0),8(a1)
...
Obj4B_Main:				; XREF: Obj4B_Index
		...
		cmpi.w	#0,($FFFFFE20).w ; do you have	at least 0 rings?
...
; ===========================================================================
Ani_obj45:
		dc.w byte_BCEB-Ani_obj45
		dc.w byte_BCEC-Ani_obj45
byte_BCEB:	dc.b 7,	0, 1, $FC
byte_BCEC:	dc.b 1,	2, 3, $FF
		even
; ---------------------------------------------------------------------------
; Sprite mappings - missile that Buzz Bomber throws
; ---------------------------------------------------------------------------
Map_obj45:
		dc.w byte_BCEE-Map_obj45, byte_BCEF-Map_obj45
		dc.w byte_BCF0-Map_obj45, byte_BCFF-Map_obj45
byte_BCEE:	dc.b 1
		dc.b $F8, 5, 0,	$24, $F8
byte_BCEF:	dc.b 1
		dc.b $F8, 5, 0,	$28, $F8
...
; ---------------------------------------------------------------------------
Obj34_ConData:	dc.w 0,	$120, $FEFC, $13C, $414, $154, $1EC, $12C ; GHZ
		dc.w 0,	$120, $FEF4, $134, $40C, $14C, $1EC, $12C ; LZ
		dc.w 0,	$120, $FEE0, $120, $3F8, $138, $1EC, $12C ; MZ
		dc.w 0,	$120, $FEFC, $13C, $414, $154, $1EC, $12C ; SLZ
		dc.w 0,	$120, $FF04, $144, $41C, $15C, $1EC, $12C ; SYZ
		dc.w 0,	$120, $FF04, $144, $41C, $15C, $1EC, $12C ; SBZ
...
ObjectFall:
		...
		add.l	d0,xpos(a0)
		move.w	yvel(a0),d0
		addi.w	#$38,yvel(a0)	; increase vertical speed
		ext.l	d0
		asl.l	#8,d0
		add.l	d0,ypos(a0)
		rts
...
SpeedToPos:
		...
		add.l	d0,xpos(a0)	; update x-axis	position
		move.w	yvel(a0),d0	; load vertical	speed
		ext.l	d0
		asl.l	#8,d0		; multiply by $100
		add.l	d0,ypos(a0)	; update y-axis	position
		rts
...
Obj47_Main:				; XREF: Obj47_Index
		...
		move.b	#1,$18(a0)
		move.b	#$D7,$20(a0)
;		move.b	#$86,$20(a0)	; Sonic get hurts when you touch the object
...
Obj56_Main:				; XREF: Obj56_Index
		...
		move.w	#$4000,2(a0)
		cmpi.b	#1,($FFFFFE10).w ; check if level is LZ
		bne.s	loc_102C8        ; branch if not equal
...
; ---------------------------------------------------------------------------
; Vladikcomper:	this code was absolutely rewriten
...
MusicList2:	incbin	misc\muslist2(Z80).bin
	endif
		even
; ===========================================================================
...
Sonic_LevelBound:			; XREF: Obj01_MdNormal; et al
		move.w	xpos(a0),d1
		move.w	CamLeftBound,d0
		addi.w	#$10,d0
		cmp.w	d1,d0          			 ; has Sonic touched the side boundary
		bhi.s	Boundary_Sides 		 ; if yes, branch
		move.w	CamRightBound,d0
		addi.w	#320-$10,d0
		tst.b	LockScreen      			; boss mode
		bne.s	loc_13332       		; if yes, branch
		addi.w	#$40,d0

loc_13332:
		cmp.w	d1,d0           			; has Sonic touched the side boundary
		blo.s		Boundary_Sides  		; if yes, branch

loc_13336:
		move.w	($FFFFF72E).w,d0
		addi.w	#$E0,d0
		cmp.w	ypos(a0),d0     		; has Sonic touched the bottom boundary
...
Obj3D_ShipStart:			; XREF: Obj3D_ShipIndex
		move.w	#$120,$12(a0)	; move ship down
		bsr.w	BossMove
		cmpi.w	#$338,$38(a0)
		bne.s	loc_177E6
...
loc_177E6:
		move.b	$3F(a0),d0
		jsr	(CalcSine).l
		asr.w	#8,d0           ; Ship Airhumping Stop (6)
		add.w	$38(a0),d0
		move.w	d0,$C(a0)
		move.w	$30(a0),8(a0)
		addq.b	#2,$3F(a0)
...
Obj3D_MakeBall:				; XREF: Obj3D_ShipIndex
		move.w	#-$100,$10(a0)  ; Speed Up (first)
		move.w	#-$10,$12(a0)   ; Ship raise up (40)
		bsr.w	BossMove
		cmpi.w	#$2A00,$30(a0)  ; Eggman is now X=2A00?
		bne.s	loc_17916       ; if yes go to loc_17916
		move.w	#-$300,$10(a0)
		move.w	#0,$12(a0)
		addq.b	#2,$25(a0)
		jsr	(SingleObjLoad2).l
...
Obj3D_ShipMove:				; XREF: Obj3D_ShipIndex
		...
		bpl.s	Obj3D_Reverse
		addq.b	#2,$25(a0)
		move.w	#$3F,$3C(a0)    ; Stay Time (Right)
		move.w	#$B00,$10(a0)	; move the ship	sideways (Speed)
		cmpi.w	#$2A00,$30(a0)  ; Eggman is now X=2A00?
		bne.s	Obj3D_Reverse   ; if yes go to Obj3D_Reverse
		move.w	#$FF,$3C(a0)    ; Add 7F for Left (First Part)
		move.w	#$FF,$10(a0)    ; Add 40 for Left (First Part)
...
loc_17960:
		bchg	#0,$22(a0)
		move.w	#$4F,$3C(a0)   ; Stay Time (Left)
		subq.b	#2,$25(a0)
		move.w	#0,$10(a0)
...
loc_17BE0:
		...
		cmp.b	$3C(a1),d0
		bne.s	loc_17BFA
		movea.l	$4(a0),a1  ; Make a stop to swinging. (34)
		cmpi.b	#6,$25(a1)
		bne.s	loc_17BFA
		addq.b	#2,$24(a0)
...
loc_18392:				; XREF: loc_1833E
		moveq	#100,d0         ; add 1000 points when you defeated.
		bsr.w	AddPoints
		move.b	#4,$25(a0)
		move.w	#$B4,$3C(a0)
		clr.w	$10(a0)
...
loc_183CA:				; XREF: off_183C2
		tst.w	$10(a0)
		bne.s	loc_183FE       ; branch if not equal
		moveq	#$40,d0
		cmpi.w	#$200,$38(a0)
		beq.s	loc_183E6       ; branch if equal
		bcs.s	loc_183DE
		neg.w	d0
...
loc_1845C:
		btst	#0,$22(a0)
		beq.s	loc_18474
		cmpi.w	#$1910,$30(a0)  ; Eggman reached X = 1910?
		blt.s	locret_1849C    ; branch if less
		move.w	#$1910,$30(a0)  ; move X = 1910
		bra.s	loc_18482       ; branch loc_18482
; ===========================================================================

loc_18474:
		cmpi.w	#$1830,$30(a0)  ; Eggman reached X = 1830?
		bgt.s	locret_1849C    ; branch if greater
		move.w	#$1830,$30(a0)  ; move X = 1830 (Left)
...
Obj73_MakeLava2:			; XREF: off_183C2
		...
		move.w	$38(a0),$C(a1)
		addi.w	#$18,$C(a1)
		move.b	#$74,(a1)	; load lava ball object (Eggman Drops Fire Ball)
		move.b	#1,$28(a1)
...
Obj74_MakeFlame:			; XREF: Obj74_Index2
		subq.w	#2,$C(a0)
		bset	#7,2(a0)
		move.w	#$1C0,$10(a0)   ; Speed Increased
		clr.w	$12(a0)
		move.w	8(a0),$30(a0)
		move.w	$C(a0),$38(a0)
		move.b	#3,$29(a0)
...
Obj7A_MakeBall:				; XREF: Obj7A_ShipIndex
		cmpi.b	#$28,$3C(a0)    ; Eggman ready to falling spikes?
		bne.s	loc_18B36       ; if yes, stop just for a moment.
		moveq	#-1,d0
		move.b	$28(a0),d0
		ext.w	d0
		bmi.s	loc_18B40
...
Obj75_Main:				; XREF: Obj75_Index
		...
		move.w	$C(a0),$38(a0)
		move.b	#$F,$20(a0)
		move.b	#$17,$21(a0)	; set number of	hits to	16
...
loc_19258:				; XREF: loc_19202
		moveq	#100,d0 ; 1000 ponits gives
		bsr.w	AddPoints
		move.b	#6,$25(a0)
		move.w	#$B4,$3C(a0)
		clr.w	$10(a0)
...
loc_19270:				; XREF: Obj75_ShipIndex
		move.w	$30(a0),d0
		move.w	#$340,$10(a0)   ; Speed Up 2
		btst	#0,$22(a0)
		bne.s	loc_1928E
		neg.w	$10(a0)
		cmpi.w	#$2BEF,d0 ; Eggman reached X = 2C12?
		bgt.s	loc_192AE ; Go to loc_192AE immediately (Go left first without regarding Sonic poistion)
		bra.s	loc_19294
; ===========================================================================

loc_1928E:
		cmpi.w	#$2D38,d0 ; Eggman reached X = 2D38?
		blt.s	loc_1929E
...
loc_192AE:
		...
		tst.b	$3D(a0)
		bne.s	loc_192E8
;		move.w	($FFFFD008).w,d1 ; searching to sonic (No searching this time)
;		subi.w	#$2C00,d1        ; No matter where Sonic stands, keep the breaking block
		asr.w	#5,d1
		cmp.b	$38(a0),d1       ; Boss Block ?
		bne.s	loc_192E8        ; if yes branch
		moveq	#0,d0
		move.b	$34(a0),d0
		asl.w	#5,d0
		addi.w	#$2B60,d0        ; Eggman pick up for break first block
		move.w	d0,$30(a0)
		bsr.w	Obj75_FindBlocks
		addq.b	#2,$25(a0)
		clr.w	$28(a0)
...
loc_19302:				; XREF: off_192FA
		move.w	#$380,$12(a0)   ; Down Speed (180)
		move.w	$38(a0),d0
		cmpi.w	#$556,d0
		bcs.s	loc_19344
		move.w	#$556,$38(a0)   ; Do not edit
		clr.w	$3C(a0)
		moveq	#-1,d0
		move.w	$36(a0),d0
		beq.s	loc_1933C
...
loc_19438:
		add.w	$0(a0),d0      ; Lift Up Nomrally
		move.w	d0,$C(a0)
		move.w	$30(a0),8(a0)
...
Obj75_FindLoop:
		...
		bne.s	loc_1946A	; if yes, branch
;		cmp.b	$38(a1),d2      ; delete first block to last block
;		bne.s	loc_1946A       ;
		move.w	a1,$36(a0)      ; Do Not Delete
		bra.s	locret_19472    ; Do Not Delete
; ===========================================================================

loc_1946A:
		lea	$40(a1),a1	; next object RAM entry (Only 40)
...
loc_19E90:				; XREF: off_19E80
		tst.l	($FFFFF680).w
		bne.s	loc_19EA2       ; branch if not equal
		cmpi.w	#$2450,($FFFFF700).w      ; Are you X = 2450?
		bcs.s	loc_19EA2                 ; if not, do not anything
		addq.b	#2,$34(a0)                ; Start boss fight
...
loc_19EA8:				; XREF: off_19E80
		...
		bpl.s	loc_19F10
		clr.w	$30(a0)
		jsr	(RandomNumber).l    ; crushing	cylinder object makes Random
		andi.w	#$C,d0
		move.w	d0,d1
		addq.w	#2,d1
		tst.l	d0
...
loc_19EC6:      ; Decision to Eggman Location
		...
		clr.b	$35(a0)
		move.w	#$B7,d0
		jsr	(PlaySound_Special).l ;	play rumbling sound (moving sound)
...
loc_19FBC:
		move.b	#6,$34(a0)
		move.w	#$25C0,8(a0)     ; Unknown
		move.w	#$53C,$C(a0)     ; Unknown
		move.b	#$14,$16(a0)
		rts
; ===========================================================================
...
loc_1A02A:				; XREF: off_19E80
		...
		jsr	(SpeedToPos).l        ; x
		move.b	#6,$1A(a0)        ; x
		addi.w	#$10,$12(a0)        ; Unknown
		cmpi.w	#$59C,$C(a0)        ; Unknown
		bcs.s	loc_1A070         ; x
		move.w	#$59C,$C(a0)      ; x
		addq.b	#2,$34(a0)        ; x
		move.b	#$20,$17(a0)      ; x
...
loc_1A074:				; XREF: off_19E80
		bset	#0,$22(a0)        ; x
		move.b	#4,$1C(a0)        ; Unknown
		jsr	(SpeedToPos).l        ; x
		addi.w	#$10,$12(a0)      ; x
		cmpi.w	#$5A3,$C(a0)      ; x
		bcs.s	loc_1A09A         ; x
		move.w	#-$40,$12(a0)     ; Unknown

loc_1A09A:
		move.w	#$400,$10(a0)     ; Unknown
		move.w	8(a0),d0          ; x
		sub.w	($FFFFD008).w,d0  ; x
		bpl.s	loc_1A0B4         ; x
		move.w	#$500,$10(a0)     ; Unknown
		bra.w	loc_1A0F2         ; x
; ===========================================================================
...
loc_lA250:
		clr.b	($FFFFFE30).w
		move.w	#1,($FFFFFE02).w ; restart level
		move.w	#$301,($FFFFFE10).w ; set level	number to 0301 (SZ2)
...
Obj86_Main:				; XREF: Obj86_Index
		move.w	#$2588,8(a0)
		move.w	#$53C,$C(a0)    ; Generator Head Sprite
		move.w	#$300,2(a0)
...
Obj86_Loop:
		jsr	(SingleObjLoad2).l
		bne.w	loc_1A954
		move.b	#$86,(a1)       ; Unknown
		move.w	8(a0),8(a1)
		move.w	#$53C,$C(a1)    ; Unknown
		move.b	#8,$24(a1)
		move.w	#$2300,2(a1)    ; Ball Sprite
		move.l	#Map_obj86a,4(a1)
		move.b	#$C,$16(a1)     ; Unknown
		move.b	#$C,$17(a1)     ; Unknown
		move.b	#0,$20(a1)
		move.b	#3,$18(a1)
		move.w	#$3E,$28(a1)    ; Unknown
		move.b	#4,1(a1)        ; Unknown
		bset	#7,1(a1)        ; Unknown
		move.l	a0,$34(a1)
		jsr	(RandomNumber).l
		move.w	$32(a0),d1      ; Balls range 1
		muls.w	#-$4F,d1        ; Balls range 2
		addi.w	#$2578,d1       ; Do not touch
		andi.w	#$1F,d0
		subi.w	#$10,d0
		add.w	d1,d0
		move.w	d0,$30(a1)      ; Do not touch
		addq.w	#1,$32(a0)
		move.w	$32(a0),$38(a0)
...
loc_1A9A6:				; XREF: Obj86_Index2
		move.w	$30(a0),d0      ; Ball range
		sub.w	8(a0),d0
		asl.w	#4,d0           ; Unknown
		move.w	d0,$10(a0)
		move.w	#$14,$28(a0)    ; Balls making cool time speed up
		addq.b	#2,$25(a0)
		rts
; ===========================================================================
...
loc_1A9E6:
		...
		addq.b	#2,$25(a0)
		move.b	#1,$1C(a0)
		move.b	#$9A,$20(a0)      ; Unknown
		move.w	#$B4,$28(a0)      ; Time set B4 for the landing on the ground
		moveq	#0,d0
		move.w	($FFFFD008).w,d0
		sub.w	8(a0),d0
		move.w	d0,$10(a0)
		move.w	#$1C0,$12(a0)     ; Down speed up
...
Obj21_Index:
		...
		dc.w Obj21_Down-Obj21_Index
		dc.w Obj21_Animate-Obj21_Index
; ===========================================================================
...
Obj21_Main:
		...
		move.w	#$90,obX(a0)
		move.w	#$C8,obScreenY(a0)

Obj21_Down:
		addq.w	#1,obScreenY(a0)
		cmpi.w	#$E8,obScreenY(a0)
		bne.s	Obj21_Animate
		addq.b	#2,obRoutine(a0)
...
Hud_10:		dc.l 10			; XREF: ContScrCounter; Hud_Secs
...
Nem_Ball:			incbin	artnem\ghzball.bin	; GHZ giant ball
		even
Nem_BallAim:		incbin	artnem\aim.bin	; GHZ aim for ball
		even
...
Nem_Lives:					incbin	artnem\lifeicon.bin				; life counter icon
		even
Nem_RobotnikLives:			incbin	artnem\robotniklives.bin			; boss life counter icon
		even
Nem_RobotnikShipCrane:		incbin	"artnem/Robotnik Ship Crane.bin"
		even
Nem_RobotnikRun:			incbin	"artnem/Robotnik Run.bin"
		even
...
Blk16_GHZ:	incbin	map16\ghz.bin
		even
Nem_GHZ_1st:	incbin	artkos\8x8ghz.bin	; GHZ primary patterns
		even
Blk256_GHZ:	incbin	map256\ghz.bin
		even
Blk16_LZ:	incbin	map16\lz.bin
		even
Nem_LZ:		incbin	artkos\8x8lz.bin	; LZ primary patterns
		even
Blk256_LZ:	incbin	map256\lz.bin
		even
Blk16_MZ:	incbin	map16\mz.bin
		even
Nem_MZ:		incbin	artkos\8x8mz.bin	; MZ primary patterns
		even
Blk256_MZ:	incbin	map256\mz.bin
		even
Blk16_SLZ:	incbin	map16\slz.bin
		even
Nem_SLZ:	incbin	artkos\8x8slz.bin	; SLZ primary patterns
		even
Blk256_SLZ:	incbin	map256\slz.bin
		even
Blk16_SYZ:	incbin	map16\syz.bin
		even
Nem_SYZ:	incbin	artkos\8x8syz.bin	; SYZ primary patterns
		even
Blk256_SYZ:	incbin	map256\syz.bin
		even
Blk16_SBZ:	incbin	map16\sbz.bin
		even
Nem_SBZ:	incbin	artkos\8x8sbz.bin	; SBZ primary patterns
		even
...
PSG01:		incbin	"sound/psg/psg1.bin"
PSG02:		incbin	"sound/psg/psg2.bin"
PSG03:		incbin	"sound/psg/psg3.bin"
PSG04:		incbin	"sound/psg/psg4.bin"
PSG05:		incbin	"sound/psg/psg5.bin"
PSG06:		incbin	"sound/psg/psg6.bin"
PSG07:		incbin	"sound/psg/psg7.bin"
PSG08:		incbin	"sound/psg/psg8.bin"
PSG09:		incbin	"sound/psg/psg9.bin"
	even
...
@0	rept	32
	...
	jmp	(@ProcessBlock).l			; process next bloku!
...
Clear_Palette:
		...
		moveq	#0,d0
		moveq	#$1F,d1
		move.l	d0,($FFFFF630).w
		move.l	d0,($FFFFF634).w
...
; ---------------------------------------------------------------------------
; Subroutine to bosses
; ---------------------------------------------------------------------------

		include "#Screens/Sega Screen/Sega Screen.asm"
		include "#Screens/Splash Screen/Splash Screen.asm"
		include "#Screens/Title Screen/Title Screen.asm"
		include "#Screens/Options Screen/Options Screen.asm"
		include "#Screens/Advanced Screen/Advanced Screen.asm"
		include "#Screens/Credits Screen/Credits Screen.asm"
		include "#Bosses/Obj8D.asm"

; =============== S U B R O U T I N E =======================================

	if Snd_Driver_Version=1
; Music
		include "z80 driver/Music/Music.asm"
	even
	endif
...
ErrorHandler:
		stopZ80
		incbin    "ErrorHandler/ErrorHandler.bin"
